...
static LIST_HEAD(timers);                                       //全局定时器链表
static int tick = 0;                                            //全局tick计数器(ms级别)
static pthread_mutex_t lock = PTHREAD_MUTEX_INITIALIZER;        //保护timers定时器链表
static pthread_rwlock_t rwlock = PTHREAD_RWLOCK_INITIALIZER;    //保护tick计数器
...
void timer_oneshot(uint32_t expire, void *(*handler)(void *), void *arg)
{
    ...
    t->refcnt = 1;                  //将新创建的定时器引用计数
    t->expires = tick + expire;     //设置过期时刻
    t->cancelled = 0;               //取消标志位置为0
